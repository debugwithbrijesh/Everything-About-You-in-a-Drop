...
st.set_page_config(page_title="Brijesh Gupta's Everything about you in a drop (Offline, Colored Dashboards)", layout="wide")
...
def styled_numeric_highlight(df, threshold_std=1.5):
    ...
    def cell_style(val, col):
        try:
            ...
            if val < m - threshold_std * s:
                return "background-color: #cfeefb"
            return ""
        except Exception:
            return ""
    sty = dfc.style
...
# Welcome page (updated title + workflows)
if page == "Welcome":
    st.title("Welcome to Brijesh Gupta's Everything about you in a drop")
    st.markdown("""
    **Overview**
    
    This project analyses blood-test CSVs: it cleans the data automatically, trains a model on your chosen target column, 
    ...
    **Workflows**
    ...
    1. **Upload & Clean** — Upload your CSV file (single header row). The app will auto-clean: trim headers, drop ID columns, impute numeric missing values, and label-encode categorical columns.
    ...
    4. **Dashboards & Visuals** — Explore the cleaned dataset using colorized tables, correlation heatmap, line/scatter/histogram/box plots.
...
# Dashboards
    ...
    if st.session_state.df_clean is None:
        ...
        if not numeric_cols:
            ...
            ycol = st.selectbox("Y-axis", numeric_cols, index=1 if len(numeric_cols) > 1 else 0, key="scatter_y")
            hue_col = None
            if len(st.session_state.obj_cols) > 0:
                hue_choice = st.selectbox("Optional categorical hue (color)", ["None"] + st.session_state.obj_cols, key="hue")
                if hue_choice != "None":
                    hue_col = hue_choice
            fig2, ax2 = plt.subplots()
            if hue_col:
                categories = st.session_state.df_clean[hue_col].astype(str)
                palette = sns.color_palette("tab10", n_colors=categories.nunique())
                sns.scatterplot(x=st.session_state.df_clean[xcol], y=st.session_state.df_clean[ycol], hue=categories, palette=palette, ax=ax2, s=50, alpha=0.7)
